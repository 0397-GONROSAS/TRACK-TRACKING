...
namespace StockTracking
{
    public partial class FrmSalesList : Form
    {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (txtSalesAmount.Text.Trim() != "")
            {
                if (rbSalesEqual.Checked)
                    ...
                    MessageBox.Show("Select a range sale");
            }
            if (chDate.Checked)
                list = list.Where(x => x.SalesDate > dtDateStart.Value && x.SalesDate < dtSalesEnd.Value).ToList();
        ...
        private void CleanFilters()
        {
            ...
            dtDateStart.Value = DateTime.Today;
            dtSalesEnd.Value = DateTime.Today;
        ...
        {
            if (detail.SalesID==0)
                MessageBox.Show("Select a Sale from the table");
            else
            {
                FrmSales frm = new FrmSales();
                frm.isUpdate = true;
        ...
        {
            if (detail.SalesID == 0)
                MessageBox.Show("Please select a object");
            else
            {
                ...
                if(result == DialogResult.Yes)
                {
                    ...
                    {
                        MessageBox.Show("The has been deleted");
                        bll = new SalesBLL();
                        dto = bll.Select();
                        dataGridView1.DataSource = dto.Sales;
                        CleanFilters();
