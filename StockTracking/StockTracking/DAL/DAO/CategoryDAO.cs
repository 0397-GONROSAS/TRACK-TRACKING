...
namespace StockTracking.DAL.DAO
{
     class CategoryDAO : StockContext, IDAO<CATEGORY, CategoryDetailDTO>
    {
        ...
        public List<CategoryDetailDTO> Select()
        {
            ...
            var list = db.CATEGORY.Where(x=>x.isDeleted == false).ToList();  //Se obtiene la base de datos
        ...
        {
            List<CategoryDetailDTO> categories = new List<CategoryDetailDTO>();
            var list = db.CATEGORY.Where(x => x.isDeleted == isDeleted).ToList();  //Se obtiene la base de datos
            foreach (var item in list)
            {
                CategoryDetailDTO dto = new CategoryDetailDTO();
                dto.ID = item.ID;
